...
rdd_broadcast = sc.broadcast(partition)
...
rdd_partition = sc.broadcast(partition)
rdd_partition
...
tweet_count_RDD=tweets.flatMap(lambda x: [(x[0],z,1) for z in list(set(tok.tokenize(x[1])))])                .distinct()                .map(lambda x: (x[1],x[2]))                .reduceByKey(lambda x,y: x + y)
tweet_count_RDD.count()
...
RDD_filter_100=tweets.flatMap(lambda x: [(x[0],z,1) for z in list(set(tok.tokenize(x[1])))])                .distinct()                .map(lambda x: (x[1],x[2]))                .reduceByKey(lambda x,y: x + y)                .filter(lambda x: users_100(x))
...
#tuple(group, token), and account
#reduce by key off of groups and tokens
#lamda function uisng the relative popularty


# In[20]:

RDD_filter_100.take(5)
...
tokens_list=tweets.flatMap(lambda x: [(x[0],z) for z in list(set(tok.tokenize(x[1])))]).map(lambda x: (x[0],[x[1]]))                                # (UserID,[Token]) 

group_tweetID=tweets.map(lambda x: (x[0],define_partition(x[0]))) #UserID & Group Number

group_key_ct=group_tweetID.join(tokens_list).filter(lambda x: x[0]>0).values().map(lambda x: ((x[0],x[1][0]),1)).reduceByKey(lambda a,b : a+b)  #joins on User ID, token and group reduce

group_key_ct.take(5)   #Group, Token, # of Each


# In[23]:

RDD_popularity=group_key_ct.map(lambda x: (x[0][1],[x[0][0],x[1]])).join(RDD_filter_100).map(lambda x: (x[0],get_rel_popularity(x[1][0][1],x[1][1]),x[1][0][0])) #popularity
RDD_popularity.take(5)
